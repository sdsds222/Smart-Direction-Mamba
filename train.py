...
from mamba_de import (
    DirectionDatasetBERT,
    MambaDirectionEstimator,
    train_mamba_de,
)
...
def _labels_from_dataset(ds: DirectionDatasetBERT) -> np.ndarray:
    return np.array(ds.labels, dtype=np.int64)
...
def train_model(csv_path: str, config: dict):
    print("\n" + "="*70)
    print("🚀 开始训练（分层划分 + AMP + EarlyStopping + 不平衡对策）")
    print("="*70)
    ...
    print("\n[3/4] 开始训练（调用 mamba_de.train_mamba_de）...")
...
def build_cli():
    p = argparse.ArgumentParser(
        description='交互式训练 Mamba Direction Estimator (多数据集 + 预训练Embedding + 分层划分)'
    )
    ...
    p.add_argument('--model-name', type=str, default='best_model', help='保存名称（会保存到 checkpoints/{name}.pth）')
    return p
...
def main():
    ...
    print("="*70)
    print("🎓 Mamba Direction Estimator - 交互式训练 (多数据集 + 预训练Embedding + 分层划分)")
    print("="*70)
