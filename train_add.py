...
def fine_tune(base_model_path: str, data_csv: str, cfg: dict):
    print("\n" + "="*70)
    print("🔄 增量训练 (Fine-tuning) - 分层划分 / AMP / EarlyStopping / 类别不平衡对策")
    print("="*70)
    ...
    trained_model, _ = train_mamba_de(
        model=model,
...
def build_cli():
    p = argparse.ArgumentParser(description='增量训练 - 在已有模型基础上继续训练（支持多语种预训练Embedding）')
...
def main():
    ...
    print("="*70)
    print("🔄 Mamba Direction Estimator - 增量训练（多语种对齐 / AMP 兼容 / 分层切分）")
    print("="*70)
